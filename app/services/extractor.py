...
async def extract_preferred(conversation: str) -> list[EntryResponse]:
...
"""
    ...
    response_text = await _call_ollama(system_prompt, user_content, temp=0.1)
    ...
    return [
        EntryResponse(
            ...
            is_system=False,
        ) for item in result.get("items", []) if item.get("mentioned") is True
    ]


async def extract_custom(conversation: str) -> list[EntryResponse]:
...
"""
    ...
    response_text = await _call_ollama(system_prompt, user_content, temp=0.2)
    result = _extract_json_from_text(response_text)
...
async def extract_all(conversation: str) -> list[EntryResponse]:
    preferred = await extract_preferred(conversation)
    custom = await extract_custom(conversation)
    return preferred + custom


async def validate_critical(extracted_items: list[EntryResponse]) -> list[EntryResponse]:
...
예시:
- "약을 안 먹었나? 기억이 안 나는데" → 미복용
- "약을 먹지 말라고 했나?" → 먹지 말라는 건지 확인
- "우울해서 죽고 싶어" → 위험
...
"""
    ...
    #검증 위한 AI 응답임.
    response_text = await _call_ollama(system_prompt, user_content, temp=0.0)
